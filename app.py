...
def load_data():
    ...
    with left_col:
        ...
        if congestion > 50000:
            ...
            fig.add_annotation(x=5, y=5.5, text="혼잡 구간 집중 관리", showarrow=True, font=dict(color="red"))

        elif congestion > 30000:
            status_text = f"{station}역 혼잡도 높음 (분산 유도 필요)"
            fig.add_annotation(x=5, y=5.5, text="우회 동선 권장", showarrow=True, font=dict(color="orange"))
